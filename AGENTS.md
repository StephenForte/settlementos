...
| ForteL2 Phase 7/8 **re-genesis** | Wipes the L2 state. Every ForteL2 contract address above expires, including the backed-up overlay's contract entries. Expired does not mean empty: a shifted deployer nonce can leave an old address holding a different live contract, so a stale reference returns wrong data instead of failing — revalidate code, type and decimals rather than assuming an address is dead. The deploy preflight's on-chain `decimals()` check catches a reassigned token whose decimals differ, but not a swap between two tokens with equal decimals (mockUSDC and mockSGD are both 6dp). The settlementos-explorer repo's address book (**11 ForteL2 rows**) becomes wrong on a public site and must be republished. After re-genesis: deploy or `--adopt` with a new `ADOPTABLE_NETWORKS` entry, then update the explorer book. Generated wallet keys in any old ForteL2 overlay backup are then worthless for signing on the new chain even if the files survive. |
