...
| ForteL2 Phase 7/8 **re-genesis** | Wipes the L2 state. Every ForteL2 contract address above is not reliably dead — a shifted deployer nonce may leave a slot empty or occupied by a different live contract, so a cached address can succeed with wrong data until identity is re-verified (not merely on error), including the backed-up overlay's contract entries; the deploy preflight's on-chain `decimals()` check catches reassigned tokens with differing decimals but not swaps between tokens with equal decimals (e.g. mockUSDC and mockSGD are both 6dp). The settlementos-explorer repo's address book (**11 ForteL2 rows**) becomes wrong on a public site and must be republished. After re-genesis: deploy or `--adopt` with a new `ADOPTABLE_NETWORKS` entry, then update the explorer book. Generated wallet keys in any old ForteL2 overlay backup are then worthless for signing on the new chain even if the files survive. |
